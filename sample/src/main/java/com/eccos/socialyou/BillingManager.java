...
public class BillingManager implements PurchasesUpdatedListener {
    ...
    public BillingManager(Activity activity) {
        ...
        Log.e("TAG", mActivity.getComponentName().getShortClassName());
    ...
    private void onQueryPurchasesFinished(PurchasesResult result) {
        ...
        String className= mActivity.getComponentName().getShortClassName();

        if(className.equals(".MainActivity")) {
            if(result.getPurchasesList().isEmpty()){
                MainActivity.Companion.setFreeUser(true);
            }
            else{
                        ...
                        new SkuDetailsResponseListener() {
                            ...
                            public void onSkuDetailsResponse(BillingResult billingResult, List<SkuDetails> skuDetailsList) {
                                if (billingResult.getResponseCode() == BillingResponseCode.OK && skuDetailsList != null) {
                                    MainActivity.Companion.setSkuDetails(skuDetailsList.get(0));

                                    for (SkuDetails skuDetails : skuDetailsList) {
                                        skuName = skuDetails.getSku();
                                        skuPrice = skuDetails.getPrice();
                                    }
                                }
                            }
                        });
    ...
    private void setSkuDetailsList(List<SkuDetails> sdl) { skuDetailsList= sdl; }

    public List<Purchase> getmPurchases() { return mPurchases; }

    public List<SkuDetails> getSkuDetailsList(){ return skuDetailsList; }

    public BillingClient getmBillingClient() { return mBillingClient; }

    /**
     * Returns the value Billing client response code or BILLING_MANAGER_NOT_INITIALIZED if the
     * clien connection response was not received yet.
     */
    public int getBillingClientResponseCode() { return mBillingClientResponseCode; }

    /**
     * Listener for the Billing client state to become connected
     */
    public interface ServiceConnectedListener { void onServiceConnected(@BillingResponseCode int resultCode);}
